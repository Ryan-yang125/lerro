...
private struct FileMetadata: Sendable {
    ...
    /// The linked file size advertised by the Hub, when available.
    ...
    init(commitHash: String?, etag: String?, linkedSize: Int64? = nil) {
        self.commitHash = commitHash
        self.etag = etag
        normalizedEtag = etag.map(Self.normalizeEtag)
        self.linkedSize = linkedSize
    }
    ...
    init?(response: URLResponse) {
        ...
        self.init(commitHash: commitHash, etag: etag, linkedSize: linkedSize)
    }
        ...
        if let cache, let etag = preflightMetadata?.normalizedEtag {
            ...
            return try await lock.withLock {
                if let expectedSize = preflightMetadata?.linkedSize,
                   ...
                   fileSizeIfExists(at: blobPath) != expectedSize {
                ...
                while true {
                    ...
                    if let commitHash = preflightMetadata?.commitHash ?? responseMetadata?.commitHash {
                        ...
                        if let cachedPath = cache.cachedFilePath(
                            ...
                            let resolved = try copyFileToDestinationIfNeeded(
                                cachedPath,
                                destination: destination
                            )
                            try? FileManager.default.removeItem(at: tempURL)
