...
struct HubCacheTests {
    ...
    @Test("Store file consumes temporary download sources")
    func storeFileConsumesTemporarySource() async throws {
        let cache = HubCache(cacheDirectory: tempDirectory)
        let repoID: Repo.ID = "user/repo"
        let commitHash = "abc123def456789012345678901234567890abcd"
        let etag = "download-etag"
        let content = Data("downloaded model bytes".utf8)
        let firstSource = tempDirectory.appendingPathComponent("first-download.tmp")
        let duplicateSource = tempDirectory.appendingPathComponent("duplicate-download.tmp")
        try content.write(to: firstSource)
        ...
        let cachedPath = try #require(cache.cachedFilePath(
            repo: repoID,
            kind: .model,
            revision: commitHash,
            filename: "model.bin"
        ...
        #expect(try Data(contentsOf: cachedPath) == content)
        ...
        try await cache.storeFile(
            ...
            kind: .model,
            revision: commitHash,
            filename: "model-copy.bin",
            etag: etag,
            consumeSource: true
        )

        #expect(!FileManager.default.fileExists(atPath: duplicateSource.path))
        #expect(try Data(contentsOf: cachedPath) == content)
    }

    @Test("Store file preserves a temporary source when snapshot commit fails")
    func storeFilePreservesSourceWhenSnapshotCommitFails() async throws {
        let cache = HubCache(cacheDirectory: tempDirectory)
        let repoID: Repo.ID = "user/repo"
        let commitHash = "abc123def456789012345678901234567890abcd"
        let source = tempDirectory.appendingPathComponent("snapshot-failure.tmp")
        let content = Data("recoverable snapshot bytes".utf8)
        try content.write(to: source)

        let blockedParent = cache.snapshotsDirectory(repo: repoID, kind: .model)
            .appendingPathComponent(commitHash)
        ...
        await #expect(throws: Error.self) {
            try await cache.storeFile(
                ...
                kind: .model,
                revision: commitHash,
                filename: "blocked/model.bin",
                etag: "snapshot-failure-etag",
                consumeSource: true
            )
        }
    ...
    @Test("Store file preserves a temporary source when ref commit fails")
    func storeFilePreservesSourceWhenRefCommitFails() async throws {
        let cache = HubCache(cacheDirectory: tempDirectory)
        let repoID: Repo.ID = "user/repo"
        let commitHash = "abc123def456789012345678901234567890abcd"
        let source = tempDirectory.appendingPathComponent("ref-failure.tmp")
        let content = Data("recoverable ref bytes".utf8)
        try content.write(to: source)

        let blockedParent = cache.refsDirectory(repo: repoID, kind: .model)
            .appendingPathComponent("blocked")
        try FileManager.default.createDirectory(
            at: blockedParent.deletingLastPathComponent(),
            withIntermediateDirectories: true
    ...
    @Test("Store file replaces a truncated blob before publishing a snapshot")
    func storeFileReplacesTruncatedBlob() async throws {
        let cache = HubCache(cacheDirectory: tempDirectory)
        let repoID: Repo.ID = "user/repo"
        let commitHash = "abc123def456789012345678901234567890abcd"
        let etag = "truncated-etag"
        let source = tempDirectory.appendingPathComponent("complete-download.tmp")
        let content = Data("complete model payload".utf8)
        try content.write(to: source)

        let blobsDirectory = cache.blobsDirectory(repo: repoID, kind: .model)
        try FileManager.default.createDirectory(
            at: blobsDirectory,
            withIntermediateDirectories: true
        )
        let blobPath = blobsDirectory.appendingPathComponent(etag)
        try Data("truncated".utf8).write(to: blobPath)
        ...
        #expect(!FileManager.default.fileExists(atPath: source.path))
        #expect(try Data(contentsOf: cachedPath) == content)
        let stagingRoot = blobsDirectory.appendingPathComponent(".staging")
