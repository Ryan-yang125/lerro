...
@testable import HuggingFace
    ...
    struct FileOperationsTests {
        ...
        func testDownloadFileResumeRangeIgnoredReturnsFullBody() async throws {
        ...
        #if !canImport(FoundationNetworking)
            @Test("downloadFile normalizes pre-start cancellation", .mockURLSession)
            func testDownloadFileNormalizesPreStartCancellation() async throws {
                let body = Data("cancelled response".utf8)
                await MockURLProtocol.setHandler { request in
                    let response = HTTPURLResponse(
                        url: request.url!,
                        statusCode: 200,
            ...
            @Test("downloadFile replaces a truncated cached blob when linked size differs", .mockURLSession)
            func testDownloadFileReplacesTruncatedCachedBlob() async throws {
                let commit = "1234567890123456789012345678901234567890"
                let body = Data("complete linked model bytes".utf8)
                let repoID: Repo.ID = "user/model"

                await MockURLProtocol.setHandler { request in
                    #expect(request.url?.path == "/user/model/resolve/main/model.bin")
                    let response = HTTPURLResponse(
                        url: request.url!,
                        statusCode: 200,
                        httpVersion: "HTTP/1.1",
                        headerFields: request.httpMethod == "HEAD"
                            ? [
                                "ETag": "\"truncated-cache-etag\"",
                                "X-Repo-Commit": commit,
                ...
                let blobPath = blobsDirectory.appendingPathComponent("truncated-cache-etag")
                try Data("short".utf8).write(to: blobPath)
                ...
                #expect(try Data(contentsOf: resolved) == body)
                #expect(try Data(contentsOf: blobPath) == body)
            }

            @Test("downloadFile falls back to an explicit destination after snapshot commit failure", .mockURLSession)
            func testDownloadFileFallsBackAfterSnapshotCommitFailure() async throws {
                let commit = "1234567890123456789012345678901234567890"
                let body = Data("recoverable destination bytes".utf8)
                let repoID: Repo.ID = "user/model"

                await MockURLProtocol.setHandler { request in
                    #expect(request.url?.path == "/user/model/resolve/main/blocked/model.bin")
                    let response = HTTPURLResponse(
                        url: request.url!,
                        statusCode: 200,
                        httpVersion: "HTTP/1.1",
                        headerFields: request.httpMethod == "HEAD"
                            ? ["ETag": "\"fallback-etag\"", "X-Repo-Commit": commit]
                            : ["Content-Type": "application/octet-stream", "Content-Length": "\(body.count)"]
                    )!
                    return (response, request.httpMethod == "HEAD" ? Data() : body)
                }
