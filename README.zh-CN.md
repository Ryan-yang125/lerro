...
<p align="center">
  按一次 Fn 开始，说完短暂静音后自动完成。Lerro 使用 macOS 26 原生 Apple Speech，<br>
  快速、准确地把语音写入当前光标。
</p>
...
按一次 Fn 开始，说完并短暂静音后，Quick Dictate 会自动写入当前 Mac App。你也可以改成按住说话、松开完成。
...
Lerro 1.5 写入后可以继续听取一句修改：恢复上一版、删除指定句、精确替换，或交给已选模型完成语义改写。每次修改都绑定同一应用、焦点输入框和已写入文字。实时转写、修正学习、确认后语音发送、应用语气、精确快捷语和默认使用 **Fn Space** 的指令继续保留。
